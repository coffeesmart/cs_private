...
import ftn_SIMPLE_stats_analysis_v2_minor1
import os
...
class MyApp(QMainWindow, QWidget):
    ...
    def btn_stats_analysis_Function(self):
        if not self.file_path_list :
            ...
            self.save_path = ftn_SIMPLE_stats_analysis_v2_minor1.stats_analysis(self.file_path_list, self.summary_onoff)

            if len(self.save_path) > 120 :
                save_path_split = self.save_path.split('/')[-1]
                self.label_save_path.setText('./' + save_path_split)
    ...
    # btn_reset이 눌리면 작동할 함수
    def btn_reset_Function(self):
        self.file_path = ""
        self.file_path_text = ""
        self.file_path_list = []
        self.save_path = ""
